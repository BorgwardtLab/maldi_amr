...
SINK_FILE = paste('./log/DRIAMS-A_2018_preprocessed_warped_',species,'_',Sys.Date(),'.log', sep='')
...
OUT_DIR = '/links/groups/borgwardt/Data/DRIAMS/DRIAMS-A/preprocessed_warped/2018/'
...
for (j in 1:length(list_files)){
    ...
    fid_suffix = paste(fileid,'_MALDI1',sep='')
    if (!(fid_suffix %in% species_codes)){
        cat(fid_suffix, 'not found', sep=' ')
        next
    }
...
FID_DIR = '/links/groups/borgwardt/Data/ms_diagnostics/USB/spectra_folder_for_IDRES/2018/2018_m2/'
...
for (j in 1:length(list_files)){
    ...
    # skip if spectrum is in codes_per_species file
    fid_suffix = paste(fileid,'_MALDI2',sep='')
    if (!(fid_suffix %in% species_codes)){
        cat(fid_suffix, 'not found', sep=' ')
        next
    }
