...
for SEED in 344 172 188 270 35 164 545 480 89 409; do
  ...
  for MODEL in "lr" "lightgbm"; do
    for SPECIES in "Escherichia coli" "Klebsiella pneumoniae"; do
        CMD="${MAIN} --species \"$SPECIES\" --antibiotic \"Ceftriaxone\" --model $MODEL --seed $SEED"
        run "$CMD";
    done # species

    CMD="${MAIN} --species \"Staphylococcus aureus\" --antibiotic \"Oxacillin\" --model $MODEL --seed $SEED"
    run "$CMD";
  done # models
done # seed
