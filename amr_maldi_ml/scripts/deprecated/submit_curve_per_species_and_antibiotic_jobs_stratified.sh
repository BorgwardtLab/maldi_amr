...
MAIN="poetry run python ../curves_per_species_and_antibiotic_stratified.py "

# Try to be smart: if `bsub` does *not* exist on the system, we just
# pretend that it is an empty command.
if [ -x "$(command -v bsub)" ]; then
  BSUB='bsub -W 23:59 -o "curve_per_species_and_antibiotic_stratified_%J.out" -R "rusage[mem=64000]"'
fi
