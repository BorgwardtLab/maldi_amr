...
for SEED in 344 172 188 270 35 164 545 480 89 409; do
  for TRAIN in "2015" "2016" "2017" "2016 2017" "2015 2016 2017"; do
    ...
    for MODEL in "lr" "lightgbm"; do
      for SPECIES in "Escherichia coli" "Klebsiella pneumoniae"; do
          CMD="${MAIN} --train-years $TRAIN --test-years 2018 --site $SITE --species \"$SPECIES\" --antibiotic \"Ceftriaxone\" --model $MODEL --seed $SEED"
          run "$CMD";
      done # species

      CMD="${MAIN} --train-years $TRAIN --test-years 2018 --site $SITE --species \"Staphylococcus aureus\" --antibiotic \"Oxacillin\" --model $MODEL --seed $SEED"
      run "$CMD";
