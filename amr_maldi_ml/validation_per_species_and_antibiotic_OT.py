...
if __name__ == '__main__':
    ...
    name = 'validation_per_species_and_antibiotic_OT'
    ...
    # import optimal transport matrix
    name = f'{train_site}' \
       f'_{"_".join(train_years)}' \
       f'_{args.species}' \
       f'_{args.antibiotic}' \
       f'_binned_6000' \
       '__vs_' \
       f'_{test_site}' \
       f'_{"_".join(test_years)}' \
       f'_{args.species}' \
       f'_{args.antibiotic}' \
       f'_binned_6000'
    name = name.replace(' ','_')

    # raise error if optimal transport matrix does not exist
    ot_path = '/home/weisc/deep_maldi/deep_maldi/optimal_transport/'

    if not os.path.isfile(os.path.join(ot_path, f'{name}.npy')):
        raise FileNotFoundError()

    # load optimal transport matrix and normalize
    OT = np.load(os.path.join(ot_path,f'{name}.npy'))
    OT /= OT.sum(axis=0)
    logging.info(f'Loaded optimal transport map: {name}')

    # apply optimal transport matrix to X_test
    X_test = np.dot(OT, X_test.T).T
