...
PATH_to_Bruker_reports_15 = '../Bruker_reports/'
...
PATH_to_IDRES_MLAB = './ID_RES_2015_11-12.csv'

#Path to the output file
PATH_to_Output = './2015-01-12_IDRES_AB_not_summarised.csv'
...
dicts_15_df = pd.DataFrame(dicts_all, index=[0]).transpose()
dicts_15_df['YEAR'] = '2015'

dicts_all_df = dicts_15_df
...
report_12 = pd.read_csv(os.path.join(PATH_to_Bruker_reports_15, '2015_12.csv'), error_bad_lines=False, sep=',', header=None)
report_11 = pd.read_csv(os.path.join(PATH_to_Bruker_reports_15, '2015_11.csv'), error_bad_lines=False, sep=',', header=None)
...
# Add '2015' string to TGNR to make it unique before combining it with data from other years
report_MALDI['TAGESNUMMER'] = '2015' + report_MALDI['TAGESNUMMER'].astype(str)

# Drop rows with empty TGNR
report_MALDI = report_MALDI[-report_MALDI['TAGESNUMMER'].isin(['2015N'])]
...
result_IDRES = result_IDRES[['strain', 'TAGESNUMMER', 'code', 'Value', 'A', 'Organism(best match)', 'Score1', 'Organism(second best match)', 'Score2', 'SPEZIES_MALDI', 'GENUS', 'SPEZIES_MLAB', 'MATERIAL', 'KEIM', 'AUFTRAGSNUMMER', 'STATION', 'PATIENTENNUMMER', 'GEBURTSDATUM', 'GESCHLECHT', 'EINGANGSDATUM', 'LOKALISATION', 'Piperacillin...Tazobactam', 'Meropenem', 'Ciprofloxacin', 'Cefepim', 'Cotrimoxazol', 'Ceftazidim', 'Amikacin', 'Levofloxacin', 'Imipenem', 'Tobramycin', 'Ceftriaxon', 'Colistin', 'Clindamycin', 'Amoxicillin...Clavulansaeure', 'Amoxicillin', 'Vancomycin', 'Penicillin', 'Metronidazol', 'Erythromycin', 'Tetracyclin', 'Fluconazol', 'Amphotericin.B', 'Caspofungin', 'Micafungin', 'Anidulafungin', 'Itraconazol', 'X5.Fluorocytosin', 'Voriconazol', 'Posaconazol', 'Ampicillin...Amoxicillin', 'Ertapenem', 'Cefpodoxim', 'Norfloxacin', 'Fosfomycin.Trometamol', 'Nitrofurantoin', 'Linezolid', 'Teicoplanin', 'Tigecyclin', 'Daptomycin', 'Gentamicin', 'Gentamicin.High.level', 'Aztreonam', 'Dummy', 'Minocyclin', 'Meropenem.ohne.Meningitis', 'Meropenem.bei.Meningitis', 'Ceftazidim.1', 'Chloramphenicol', 'Chinolone', 'Clarithromycin', 'Cefuroxim', 'Oxacillin', 'Cefazolin', 'Rifampicin', 'Fusidinsaeure', 'Streptomycin.1.0.mg.l', 'Rifampicin.1.0.mg.l', 'Isoniazid.0.1.mg.l', 'Pyrazinamid.100.0.mg.l', 'Ethambutol.5.0.mg.l', 'Azithromycin', 'Cefixim', 'Moxifloxacin', 'Penicillin.bei.anderen.Infekten', 'Penicillin.bei.Meningitis', 'Penicillin.bei.Pneumonie', 'Mupirocin', 'Aminoglykoside', 'Meropenem.bei.Pneumonie', 'Amoxicillin-Clavulansaeure.unkompl.HWI', 'Fosfomycin', 'Ticarcillin...Clavulansaeure', 'Penicillin.ohne.Endokarditis', 'Penicillin.bei.Endokarditis', 'Doxycyclin', 'Cefoxitin.Screen', 'Isoniazid.0.4.mg.l', 'Bacitracin', 'Cefepim.1', 'Vancomycin.GRD', 'Teicoplanin.GRD', 'PATIENTENNUMMER_id', 'FALLNUMMER_id', 'AUFTRAGSNUMMER_id']]
